...
export class RuntimeAudioClient {
  ...
  private readonly pending = new Map<
    string,
    { resolve: (value: Result<any>) => void; reject: (error: Error) => void }
  >();
  ...
  async reconnect(ws: WebSocket): Promise<AudioSession> {
    ...
    const shouldRestoreTransmission = this.transmitting;
    ...
    this.log(
      `room.reconnect OK participant=${this.session.participantId} session=${this.session.participantSessionId} mediaRecoveryRequired=true`
    );
    ...
    this.log(
      `reconnect media authorization role=${audioState.audioRole} selfMuted=${audioState.selfMuted} moderatorMuted=${audioState.moderatorMuted} canTransmitAudio=${audioState.canTransmitAudio}`
    );

    this.transmitting = false;
    if (shouldRestoreTransmission && audioState.canTransmitAudio) {
      await this.produceMicrophone();
      this.log('microphone producer recovered on the new media session');
    } else if (shouldRestoreTransmission) {
      this.log(
        'previous transmission was active, but current server state does not authorize transmission'
      );
    }
    return this.session;
  }
  ...
  private async consumeProducer(producerId: string, participantId?: string): Promise<void> {
    ...
    this.log(`consumer active id=${consumer.id} producer=${producerId}`);
  }
