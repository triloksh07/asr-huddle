...
import { connectTransportSchema, consumeAudioSchema, produceAudioSchema } from "@repo/media-contract";
...
export class MediaController {
  ...
  async produceAudio(context: MediaSessionContext, payload: unknown) {
    ...
    if (
      ...
      throw new MediaControlError("MEDIA_IDENTITY_MISMATCH", "Producer identity does not match the realtime session.");
    }
  ...
  async consumeAudio(context: MediaSessionContext, payload: unknown) {
    ...
    if (
      ...
      throw new MediaControlError("MEDIA_IDENTITY_MISMATCH", "Consumer identity does not match the realtime session.");
    }
