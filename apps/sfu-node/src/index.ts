...
async function main(): Promise<void> {
  ...
  const media = new MediasoupMediaService({ worker, mediaCodecs: CONFIG.mediaCodecs });
...
main().catch(error => { console.error("Fatal SFU startup failure.", error); process.exit(1); });
