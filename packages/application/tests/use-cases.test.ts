...
class MemoryUsers {
  ...
  async findById(id: string) { return id === this.user.id ? this.user : null; }
}

class MemoryRooms {
  private readonly data = new Map<string, ReturnType<typeof createRoom>>();
  async findById(id: string) { return this.data.get(id) ?? null; }
  async save(room: ReturnType<typeof createRoom>) { this.data.set(room.id, room); }
}

class MemorySessions {
  private readonly data = new Map<string, ReturnType<typeof createRoomSession>>();
  async findActiveByRoomId(roomId: string) {
    return [...this.data.values()].find((session) => session.roomId === roomId && session.status === "ACTIVE") ?? null;
  }
  async save(session: ReturnType<typeof createRoomSession>) { this.data.set(session.id, session); }
}
...
class MemoryParticipants {
  ...
  async save(participant: any) { this.data.set(participant.id, participant); }
}
...
class MemoryRequests {
  ...
  async findPendingByParticipantId(participantId: string) {
    return [...this.data.values()].find((request) => request.participantId === participantId && request.status === "PENDING") ?? null;
  }
  async save(request: any) { this.data.set(request.id, request); }
}

class FixedIds {
  private nextValue = 0;
  next() { this.nextValue += 1; return `id-${this.nextValue}`; }
}
...
describe("application use cases", () => {
  it("joins a user as a listener", async () => {
    ...
    const participant = await new JoinRoom(
      ...
      new FixedIds(),
      clock,
    ).execute({ roomId: "room-1", userId: "user-1" });
  ...
  it("creates one pending speaker request and rejects duplicates", async () => {
    ...
    await useCase.execute({ participantId: participant.id });
    await expect(useCase.execute({ participantId: participant.id }))
