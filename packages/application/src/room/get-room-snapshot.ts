...
export class GetRoomSnapshot {
  ...
  async execute(command: GetRoomSnapshotCommand): Promise<RoomSnapshot> {
    ...
    if (!session) throw new ApplicationError('ROOM_ENDED', 'The room has no active session.');
    const participants = await this.participants.findByRoomSession(session.id);
    const raised = new Set((await this.raisedHands?.list(session.id)) ?? []);
    return {
      room,
      session,
      participants: participants.map(participant => ({
        ...
        handRaised: raised.has(participant.id),
      })),
    };
  }
}
