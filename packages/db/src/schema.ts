...
export const roomSessionStatusEnum = pgEnum("room_session_status", [
  "ACTIVE",
  "ENDED",
]);
...
export const audioRoleEnum = pgEnum("audio_role", [
  "SPEAKER",
  "LISTENER",
]);
...
export const roomSessions = pgTable(
  ...
  (table) => [
    index("room_sessions_room_id_idx").on(table.roomId),
    index("room_sessions_status_expires_at_idx").on(table.status, table.expiresAt),
    uniqueIndex("room_sessions_one_active_per_room_idx")
      .on(table.roomId)
      .where(
        // Partial unique index is emitted by Drizzle/Postgres; only ACTIVE sessions are unique.
        // The expression is supplied through the SQL fragment in the migration below.
        undefined as never,
      ),
  ],
);
...
export const participants = pgTable(
  "participants",
  {
    id: uuid("id").defaultRandom().primaryKey(),
    roomId: uuid("room_id").notNull().references(() => rooms.id),
    roomSessionId: uuid("room_session_id").notNull().references(() => roomSessions.id),
  ...
  (table) => [
    index("participants_room_session_status_idx").on(
      table.roomSessionId,
      table.status,
    ),
...
export const participantSessions = pgTable(
  ...
  (table) => [
    index("participant_sessions_participant_status_idx").on(
      table.participantId,
      table.status,
    ),
    uniqueIndex("participant_sessions_connection_id_idx").on(table.connectionId),
  ],
);
